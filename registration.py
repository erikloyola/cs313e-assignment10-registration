...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_depth(vertex_idx):
            vertex = self.vertices[vertex_idx]
            max_depth = 0
            for neighbor_idx in self.get_adjacent_vertices(vertex_idx):
                neighbor = self.vertices[neighbor_idx]
                if neighbor.depth == -1:
                    dfs_depth(neighbor_idx)
                max_depth = max(max_depth, 1 + neighbor.depth)
    ...
    def has_cycle(self):
        ...
        """
        def dfs_cycle(vertex_idx, path_set):
            vertex = self.vertices[vertex_idx]
            vertex.visited = True
            path_set.add(vertex_idx)

            for neighbor_idx in self.get_adjacent_vertices(vertex_idx):
                neighbor = self.vertices[neighbor_idx]
                if neighbor_idx in path_set:
                    return True
                if not neighbor.visited:
                    if dfs_cycle(neighbor_idx, path_set):
                        return True

            path_set.remove(vertex_idx)
            return False
    ...
    def get_registration_plan(self):
        ...
        """
        courses = []
        in_degree = {idx: 0 for idx in range(len(self.vertices))}
        ...
        while available:
            ...
            for _ in range(min(4, len(available))):
                ...
                for neighbor_idx in self.get_adjacent_vertices(idx):
                    in_degree[neighbor_idx] -= 1
                    if in_degree[neighbor_idx] == 0:
                        next_available.append(neighbor_idx)
